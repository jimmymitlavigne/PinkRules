/* NewsBilingual 0.2.2 | Quantumult X | Generated by tools/build.cjs
...
function diagnosticHtml(config) {
  var info = publicSettings(config);
...
function main() {
  ...
  try {
    ...
    if (typeof $response !== 'undefined') {
      ...
      if (body === null && !assetPath(route) && (/html|json|xml|protobuf|octet-stream/i.test(type) || Number($response.statusCode) === 200)) {
        log('response skipped path=' + route + ' method=' + $request.method + ' status=' + $response.statusCode + ' type=' + type);
      }
      else log('button injected path=' + route + ' bytes=' + body.length);
      return done(body === null ? {} : {body: body});
    }
    if (route === PREFIX + 'client.js') {
      if ($request.method !== 'GET') throw fault(405, '仅支持 GET');
      log('client.js served host=' + match[1]);
      return reply(200, '(' + browserMain.toString() + ')(' + JSON.stringify(publicSettings(config)).replace(/</g, '\\u003c') + ');', 'application/javascript');
    }
    if (route === PREFIX + 'status') { log('status check host=' + match[1]); return reply(200, publicSettings(config)); }
    if (route === PREFIX + 'diagnostic') {
      if ($request.method !== 'GET') throw fault(405, '仅支持 GET');
      log('diagnostic page served host=' + match[1]);
      return reply(200, diagnosticHtml(config), 'text/html');
    }
